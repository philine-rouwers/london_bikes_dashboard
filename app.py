...
def dataset_metrics():
    ...
    return [
        ...
        metric_card(
            ...
            f"{month['bikes_hired']:,.2f} mean daily hires",
            C["purple"],
        ),
    ]
...
def browser_weather_to_daily(payload):
    """Convert raw Open-Meteo hourly JSON fetched in the browser to model inputs."""
    ...
    if missing:
        raise ValueError("Open-Meteo response is missing: " + ", ".join(missing))

    hf = pd.DataFrame(
        {
            "time": pd.to_datetime(hourly["time"], errors="coerce"),
            "temperature_2m": pd.to_numeric(hourly["temperature_2m"], errors="coerce"),
            "windspeed": pd.to_numeric(hourly["wind_speed_10m"], errors="coerce"),
            "solarradiation": pd.to_numeric(hourly["shortwave_radiation"], errors="coerce"),
            "sealevelpressure": pd.to_numeric(hourly["pressure_msl"], errors="coerce"),
            "visibility_m": pd.to_numeric(hourly["visibility"], errors="coerce"),
            "precip": pd.to_numeric(hourly["precipitation"], errors="coerce"),
        }
    ).dropna(subset=["time"])
    ...
    daily = (
        hf.groupby("date", as_index=False)
        .agg(
...
def model_credibility_content():
    ...
    if perf is None:
        return [
            insight_card(
                "Model fit",
                "Historical fit unavailable",
                "Historical fit statistics are unavailable because the training data could not be loaded.",
                C["amber"],
            )
        ]
    ...
    return [
        html.Div(
            [
                metric_card(
                    "R²",
                    f"{r2:.3f}",
                    "Share of variation explained by the fitted model",
                    C["blue"],
                ),
                metric_card(
                    "Adjusted R²",
                ...
                metric_card(
                    "Residual SE",
                    f"{residual_se:,.1f}",
                    "Typical residual size, in daily hires",
                    C["purple"],
                ),
                metric_card(
                    "Training days",
                    f"{n:,}",
                    "Daily observations used for model fitting",
                    C["amber"],
                ),
            ],
        ...
        insight_card(
            "Fit summary",
            f"About {adjusted_r2 * 100:.1f}% of variation explained",
            "Adjusted R² summarises explanatory power while accounting for model complexity. "
            f"The residual standard error is {residual_se:,.0f} daily hires.",
            C["teal"],
            detail=f"Model formula: {MODEL_FORMULA}",
        ),
        card(
            [
                html.H3("Actual vs fitted values", className="section-heading"),
                html.P(
                    "Each point compares observed daily hires with the model's fitted value for the same day. Points closer to the dashed 45° line indicate a closer fit.",
        ...
        [
                        ...
                        [
                            html.Div(
                                [
                                    ...
                                    html.P(
                                        "Each point represents one day. Hover for exact values; zoom to inspect a narrower range.",
            ...
            card(
                [
                    ...
                    html.P(
                        "View daily, weekly or monthly bike demand across the selected period, with an optional weather variable on the secondary axis.",
                        className="chart-help",
                    ),
                    dbc.Row(
                        [
        ...
        [
            card(
                [
                    ...
                    html.P(
                        "Each cell shows the strongest Pearson correlation between any pair of variables in the two categories. Hover to identify the variable pair, then select a cell to compare those categories in detail.",
                        className="section-copy",
                    ),
            ...
            card(
                [
                    ...
                    html.P(
                        "Compare two selected variables across historical days. The fitted line summarises their linear relationship; hover for exact values.",
                        className="chart-help",
                    ),
        ...
        [
            card(
                [
                    ...
                    html.P(
                        "Explore how daylight, lunar phase and temperature align with historical bike demand. Adjust the controls to find the closest matching days in the historical record.",
                        className="section-copy",
                    ),
                    html.Div(
                        [
            ...
            card(
                [
                    ...
                    html.P(
                        "The chart shows historical days most similar to the selected moon phase, daylight window and temperature. Similarity is descriptive and should not be interpreted as a causal effect.",
                        className="chart-help",
                    ),
...
def predict_tab():
    final_model_terms = (
        "The model estimates daily bike hires from weather and calendar conditions, "
        "including maximum temperature, wind speed, solar radiation, sea-level pressure, "
        "visibility, precipitation, weekday, season and weekend effects."
    )
    ...
    return html.Div(
        [
            card(
                [
                    html.H3("Model specification", className="section-heading"),
                    html.P(
                        final_model_terms,
                        className="section-copy",
                    ),
                    html.Div(
                        [
                            metric_card("Outcome", "Daily bike hires", "Daily cycle-hire demand", C["blue"]),
                            metric_card("Training period", "2014 onward", "Modern complete historical period", C["teal"]),
                            metric_card("Weather variables", f"{len(model_numeric_variables(COEFFS))}", "Numeric weather inputs", C["purple"]),
                            metric_card("Interaction terms", "4", "Visibility × season; precipitation × weekend", C["pink"]),
                        ],
                        className="metrics-grid",
                    ),
                ],
            ...
            card(
                [
                    html.H3("Model fit", className="section-heading"),
                    html.P(
                        "Goodness-of-fit statistics and fitted values for the OLS model on the 2014+ training period.",
                        className="section-copy",
                    ),
            ...
            card(
                [
                    html.H3("Demand forecasts from weather data", className="section-heading"),
                    html.P(
                        "The January 2026 estimate uses archived Open-Meteo observations. "
                        "The five-day forecast uses the latest available London weather forecast for the next five future dates. "
                        "Daily weather inputs are passed to the fitted regression model to estimate bike hires.",
                        className="section-copy",
                    ),
                    dbc.Button("Update weather forecast", id="refresh-predictions", n_clicks=0, color="primary"),
            ...
            dbc.Row(
                [
                    dbc.Col(
                        card(
                            [
                                html.H3("Scenario explorer", className="section-heading"),
                                html.P(
                                    "Set the weather conditions, weekday and season for a hypothetical day. "
                                    "The model then estimates daily bike hires for that scenario.",
                                    className="section-copy",
                                ),
                                dbc.Row(
                                    [
                    ...
                    dbc.Col(
                        card(
                            [
                                ...
                                html.P(
                                    "The chart shows the intercept and the 10 largest active model contributions for the selected scenario. "
                                    "Positive contributions increase the estimate; negative contributions reduce it.",
    ...
    [
        html.Div(
            [
                ...
                html.P(
                    "Explore historical London cycle-hire demand, weather relationships and model-based demand forecasts.",
                    className="hero-subtitle",
                ),
                dbc.Badge(
                    "OLS demand model · 2014 onward",
                    color="success",
        ...
        # Weather is fetched directly from the visitor's browser. This avoids
        # Open-Meteo rate limits on Render's shared outbound server IP.
        dcc.Store(id="weather-api-store"),
        ...
        html.Footer(
            [
                html.Div("Sources: London Bikes historical data · Open-Meteo weather and forecasts · OLS demand model fitted on daily data from 2014 onward", className="footer-left"),
                html.Div(
                    f"Historical data through {data_through} · Forecast weather updates on demand",
...
# Fetch Open-Meteo directly from the user's browser instead of from Render.
# Render free services share outbound IPs, which can trigger Open-Meteo HTTP 429.
app.clientside_callback(
    """
    async function(nClicks) {
        const latitude = 51.50853;
        const longitude = -0.125748;

        const CACHE_KEY = "london_bikes_openmeteo_cache_v1";
        const FORECAST_TTL_MS = 30 * 60 * 1000;   // automatic reuse for 30 minutes
        const REFRESH_THROTTLE_MS = 5 * 60 * 1000; // manual clicks within 5 min reuse cache
        ...
        let cached = null;
        try {
            cached = JSON.parse(window.localStorage.getItem(CACHE_KEY));
        } catch (e) {
            cached = null;
        }

        const now = Date.now();
        const cachedAt = cached && cached.fetched_at
            ? Date.parse(cached.fetched_at)
            : NaN;
        const age = Number.isFinite(cachedAt) ? now - cachedAt : Infinity;
        const manualRefresh = Number(nClicks || 0) > 0;
        const reuseWindow = manualRefresh ? REFRESH_THROTTLE_MS : FORECAST_TTL_MS;

        // Reuse a recent complete response. This avoids repeated API calls on
        // page reloads and rapid clicks of the update button.
        if (
            cached &&
            cached.history &&
            cached.forecast &&
        ...
        try {
            // The January 2026 archive is fixed historical data, so once it has
            // been cached successfully there is no reason to request it again.
            const historyPromise = cached && cached.history
            ...
            // If a live refresh is temporarily rate-limited or unavailable,
            // preserve the last successful forecast rather than blanking the app.
            if (cached && cached.history && cached.forecast) {
                return {
                    ...cached,
                    cache_status: "stale",
                    warning: "The latest weather update was unavailable, so the most recently saved forecast is shown.",
                    error: null
                };
            }
...
@app.callback(
    ...
    Input("weather-api-store", "data"),
)
def refresh_predictions(weather_payload):
    if COEFF_ERROR:
    ...
    if weather_payload.get("error"):
        return dbc.Alert(
            "Weather data are temporarily unavailable: "
            + str(weather_payload["error"])
            + ". Please try updating the weather forecast again in a moment.",
            color="warning",
            className="mt-3",
        )
    ...
    try:
        history = browser_weather_to_daily(weather_payload.get("history"))
        history_pred = predict_hires(history, COEFFS)
        sections.append(
            prediction_section(
                "Estimated bike hires: 1–7 January 2026",
                history_pred,
            )
        )
    except Exception as exc:
        sections.append(
            dbc.Alert(
                f"Historical prediction failed: {exc}",
                color="danger",
                className="mt-3",
            )
        )
    ...
    try:
        forecast = browser_weather_to_daily(weather_payload.get("forecast"))

        # forecast_days includes today. Remove the first calendar date and keep
        # the next five genuinely future days.
        forecast = forecast.sort_values("date").iloc[1:6].copy()

        if len(forecast) != 5:
            raise ValueError(
                f"Expected five future dates but Open-Meteo returned {len(forecast)}."
            )

        forecast_pred = predict_hires(forecast, COEFFS)
        sections.append(
            prediction_section(
                "Forecast bike hires: next five days",
                forecast_pred,
            )
        )
    except Exception as exc:
        sections.append(
            dbc.Alert(
                f"Five-day forecast failed: {exc}",
                color="danger",
                className="mt-3",
            )
        )

    fetched = weather_payload.get("fetched_at")
    if fetched:
        try:
            fetched_text = pd.to_datetime(fetched).strftime("%d %b %Y, %H:%M UTC")
        except Exception:
            fetched_text = str(fetched)

        cache_status = weather_payload.get("cache_status")
        warning = weather_payload.get("warning")

        if cache_status == "stale" or warning:
            status_text = (
                f"Weather forecast last updated {fetched_text}. "
                "A newer update was temporarily unavailable, so the most recent saved forecast is shown."
            )
            status_color = "warning"
        elif cache_status == "cached":
            status_text = (
                f"Weather forecast last updated {fetched_text}. "
                "Recent forecast data have been reused to limit unnecessary API requests."
            )
            status_color = "info"
        else:
            status_text = f"Weather forecast updated {fetched_text}."
...
def scenario_prediction(day, season, values, ids):
    ...
    pred_card = metric_card(
        "Estimated bike hires",
        f"{predicted:,.2f}",
        f"{DAY_LABELS.get(day, day)} · {season} scenario",
        C["blue"],
    )
    ...
    fig = px.bar(
        ...
        title="Largest contributions to the scenario estimate",
        labels={
            "term": "",
            "contribution": "Contribution to estimated bikes hired",
        },
    )
    fig.update_traces(
        marker_color=C["purple"],
